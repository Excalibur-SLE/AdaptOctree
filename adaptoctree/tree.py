...
def build(
    ...
    max_bound, min_bound = morton.find_bounds2(particles)
    octree_center = morton.find_center(max_bound, min_bound)
    octree_radius = morton.find_radius(octree_center, max_bound, min_bound)

    morton_keys = morton.encode_points(particles, 1, octree_center, octree_radius)
...
def build_implementation(
    ...
    # morton_keys = morton.encode_points(particles, level, octree_center, octree_radius)
    todo_indices_sorted = todo_indices[np.argsort(morton_keys[todo_indices])]

    while True:
        if level == (maximum_level):
            break
        todo_list = process_level(todo_indices_sorted, morton_keys, maximum_particles)
        ntodo = len(todo_list)
        if ntodo == 0:
            break
...
@numba.njit
def process_level(sorted_indices, morton_keys, maximum_particles):
    """Process a level."""
    count = 0
